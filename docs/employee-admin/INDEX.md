...
- `getCurrentContract` (`src/lib/utils.ts`) désigne le contrat "actuel" en prenant simplement le `startDate` le plus récent — **pas** celui réellement valide aujourd'hui. Si deux contrats se chevauchent (un contrat fermé en retard, ou un contrat saisi avec une date de début rétroactive plus ancienne qu'un contrat existant), le mauvais contrat peut être affiché comme "actuel"/"Ongoing". C'est ce qui s'est produit avec des contrats placeholder (`title: "Employee"`, `team: "Unassigned"`) créés par une ancienne page d'auto-inscription (`src/app/auth/signup/page.tsx`, supprimée depuis) : nettoyés une fois en base pour les employés concernés (2026-06-16), et la clôture auto du contrat précédent (ci-dessus) empêche la récidive. Si ce genre de décalage réapparaît, vérifier d'abord s'il existe des contrats orphelins/doublons dans `contracts[]` avant de suspecter `getCurrentContract`.
