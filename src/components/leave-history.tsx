    ...
    const getStatusBadge = (request: LeaveRequest) => {
        ...
        const badge = () => {
             switch (status) {
                case 'Approved': 
                case 'Pending HR':
                    return <><Badge variant="default" className="bg-green-500 hover:bg-green-600">Approved</Badge><Link target="blank" href={`/download/${request.id}`}><FileDownIcon className="cursor-pointer" /></Link></>;
